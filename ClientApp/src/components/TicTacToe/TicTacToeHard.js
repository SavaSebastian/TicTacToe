    ...
    useEffect(() => 
    {
        ...
        (
                ...
                const response = await fetch("https://localhost:5001/api/GetUser", 
            {
                ...
                let whoWon = calculateWinner(history[stepNumber]);
                
                let state = whoWon === "X" ? "Win" : whoWon === "O" ? "Lose" : "Draw";
        ...
        })().then(function()
            {
                (
                    async () => {
                        if((winner === "X" || winner === "O") || stepNumber === 10) 
                        {
                            await fetch("https://localhost:5001/apiMatch/AddMatch",
                                {
                                    method: "POST",
                                    headers: {"Content-Type": "application/json"},
        ...
        <Grid className={classes.Grid}>
            ...
            <table>
                <thead>
                    <tr>
                        <th>
                            <h3> {name}: <font color="red">X</font></h3>
                        </th>
                        <th>
